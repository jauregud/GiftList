...
// Helper to get all groups (needed to find group by invite code)
function getGroupByCode(code: string): Group | null {
  try {
    const all = JSON.parse(localStorage.getItem("giftlist_groups") || "[]") as Group[];
    return all.find((g) => g.inviteCode === code) ?? null;
  } catch {
    return null;
  }
}
